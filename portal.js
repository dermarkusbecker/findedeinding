...
function renderDashboardClarityChart() {
  ...
  const top = 24;
  const bottom = 236;
  const weekPoints = Array.from({length:8},(_,i)=>measurements.filter(item=>Number(item.week)===i+1).at(-1)).filter(Boolean);
  const x = (week) => left + ((Number(week) - 1) / 7) * (right - left);
  const y = (score) => bottom - ((Number(score) - 1) / 9) * (bottom - top);
  const pointPairs = weekPoints.map((item,index) => [x(item.week), y(item.score)]);
...
function renderJourney() {
  ...
  $('#journeyGrid').innerHTML = journeyWeeks.map((state) => {
    ...
    return `${phaseHeading}<article class="week-card ${phased ? 'journey-phase-' + phase : ''} ${state.completed ? 'completed' : active ? 'active' : state.accessible ? 'available' : 'locked'}" data-preview-week="${state.week}" tabindex="0" role="button"><span>Woche ${state.week}</span><i>${status}</i><h2>${escapeHtml(summary?.title || `Woche ${state.week}`)}</h2><p>${escapeHtml(summary?.description || summary?.mode || 'Dein nächster Schritt im Acht-Wochen-Prozess.')}</p><b>${reason} · Details ansehen</b></article>`;
  }).join('');
