import { chromium } from '/tmp/markus-builder-browser/node_modules/playwright/index.mjs';
...
await page.addInitScript(()=>{
 ...
 window.SpeechRecognition=class {constructor(){window.rec=this}start(){this.onstart()}stop(){this.onaudioend()}abort(){this.onend()}};
});
...
await page.evaluate(()=>{const r=[{0:{transcript:'Hallo Clara'},isFinal:true}];rec.onresult({results:r});rec.onresult({results:r});});
assert.equal(await page.locator('textarea').inputValue(),'');
await page.click('[data-speech-stop]');await page.getByText('Wird transkribiert …',{exact:true}).waitFor();
await page.evaluate(()=>rec.onend());await page.waitForFunction(()=>document.querySelector('textarea').value==='Hallo Clara');
