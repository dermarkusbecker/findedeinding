...
test('account reads scope invoices, payments and adjustments to the same customer including unassigned payments',async()=>{
 const paths=[],response=res();await handleCustomerAccount({method:'GET',query:{action:'finance-account',customerId:customer}},response,{today:'2026-09-21',user:{},query:async path=>path.startsWith('user_profiles')?[{id:customer,name:'Kunde',source_lead_id:lead}]:path.startsWith('leads')?[{id:lead}]:[{}],all:async path=>{if(path.startsWith('finance_payment_plans')){assert.ok(path.includes(`customer_id=eq.${customer}`));return [];}if(path.startsWith('finance_account_requests')){assert.ok(path.includes(`customer_id=eq.${customer}`));return [];}paths.push(path);assert.ok(path.includes(`lead_id=in.(${lead})`));return path.startsWith('lead_payments')?[{id:'payment',amount:25,status:'booked',booked_at:'2026-09-21'}]:[];}});
