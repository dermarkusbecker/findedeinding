...
export async function readClarityQuestionOverrides(service, week = null) {
  ...
  const requestRows = async (select) => {
    const response = await fetch(`${service.url}/rest/v1/clarity_questions?select=${select}${filter}&order=week.asc,sort_order.asc`, { headers: serviceHeaders(service.key) });
    const rows = await response.json().catch(() => ([]));
    return { response, rows };
  };
  const fullSelect = 'question_key,week,step_id,title,prompt_text,default_prompt_text,guidance_text,default_guidance_text,completion_criteria,default_completion_criteria,prompt_type,sort_order,enabled,updated_at';
  const legacySelect = 'question_key,week,step_id,title,prompt_text,default_prompt_text,prompt_type,sort_order,enabled,updated_at';
  let { response, rows } = await requestRows(fullSelect);
  const missingBuilderColumns = !response.ok && (['PGRST204', '42703'].includes(rows?.code) || /column .*?(guidance_text|completion_criteria).*?does not exist/i.test(rows?.message || ''));
  if (missingBuilderColumns) ({ response, rows } = await requestRows(legacySelect));
