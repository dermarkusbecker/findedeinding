...
export async function handleFinance(request,response){
 const action=request.query?.action||'',portal=action==='finance-my-invoices'||action==='finance-my-pdf';
 ...
 try{
  ...
  const invoices=await all(`finance_invoices?select=*&order=created_at.desc,id${filter}${action.endsWith('pdf')?`&id=eq.${request.query.id}`:''}`);
  ...
  const balances=invoiceBalances(invoices,payments,today);
  if(portal)return response.json({invoices:balances.map(({id,invoice_number,invoice_date,due_date,gross,net,vat,paid,open,payment_status,description})=>({id,invoice_number,invoice_date,due_date,gross,net,vat,paid,open,payment_status,description}))});
  if(action==='finance-report'){
   const report=financeReport(invoices,payments,{year:request.query.year,month:request.query.month||0,today});
  ...
  return response.json({invoices:balances,payments:request.query.customerId?payments.filter(p=>invoices.some(i=>i.lead_id===p.lead_id)):payments,today,settings:(await query('finance_settings?id=eq.default'))[0]});
