...
export async function buildCompletedPrivacyPdf(consent) {
  ...
  const signature = form.getSignature('unterschrift_teilnehmer');
  const rectangle = signature.acroField.getWidgets()[0]?.getRectangle();
  form.acroForm.removeField(signature.acroField);
  pdf.getPages()[1].node.removeAnnot(signature.ref);
  form.flatten();
  if (rectangle) {
    ...
    page.drawText(signatureText, { x: rectangle.x + 7, y: rectangle.y + 9, size: 8.6, font, color: rgb(0.08, 0.20, 0.27), maxWidth: rectangle.width - 14 });
  }
...
export async function readPrivacyConsentDocument(service, participantId) {
  const response = await fetch(`${service.url}/rest/v1/participant_documents?user_profile_id=eq.${encodeURIComponent(participantId)}&week=eq.0&document_type=eq.privacy_consent&select=id,participant_confirmed_at,extracted_data,created_at&order=created_at.desc&limit=1`, { headers: serviceHeaders(service.key) });
  const data = await response.json().catch(() => ([]));
...
export async function storePrivacyConsentDocument(service, participantId, consent, confirmedAt = new Date().toISOString()) {
  const existing = await readPrivacyConsentDocument(service, participantId);
  ...
  try {
    const rows = await responseRows(await fetch(`${service.url}/rest/v1/participant_documents`, {
      ...
      body: JSON.stringify({
        ...
        extracted_data: { consent, signatureMethod: 'digital_click_confirmation', confirmedAt },
