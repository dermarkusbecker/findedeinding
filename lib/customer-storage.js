...
const BUCKETS = Object.freeze({
  ...
  contractRecordings: { id: 'contract-recordings', limit: 500 * 1024 * 1024, types: ['video/webm', 'video/mp4', 'audio/webm'] },
});
...
export async function createSignedCustomerUpload(service, kind, ownerId, { fileName, mimeType, byteSize }) {
  ...
  if (!Number.isFinite(Number(byteSize)) || Number(byteSize) <= 0 || Number(byteSize) > bucket.limit) throw Object.assign(new Error('Die Aufzeichnung fehlt oder überschreitet 500 MB.'), { status: 413 });
