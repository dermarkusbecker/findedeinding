...
export function createSession(identity, role = 'user', claims = {}, ttlMs = 1000 * 60 * 60 * 12) {
  const permissions = role === 'admin' ? [...USER_PERMISSIONS] : USER_PERMISSIONS.filter((permission) => claims.permissions?.includes(permission));
  const lifetime = Number.isFinite(Number(ttlMs)) ? Math.max(1000 * 60, Math.min(Number(ttlMs), 1000 * 60 * 60 * 12)) : 1000 * 60 * 60 * 12;
  const payload = Buffer.from(JSON.stringify({ identity, role, ...claims, permissions, expires: Date.now() + lifetime })).toString('base64url');
...
export function sessionFromRequest(request) {
  ...
  if (previewToken) return sessionFromToken(previewToken);
  const cookie = request.headers?.cookie?.split(';').map((part) => part.trim()).find((part) => part.startsWith(`${COOKIE}=`));
  return cookie ? sessionFromToken(cookie.slice(COOKIE.length + 1)) : null;
}
