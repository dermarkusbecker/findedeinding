...
function removeSignatureWidget(pdf, form) {
  ...
  const reference = widget?.ref || signature.ref || signature.acroField.ref;
  let annotationRef = reference;
  const pageIndex = pdf.getPages().findIndex((page) => {
    const match = reference && page.node.Annots()?.asArray().find((item) => item === reference || String(item) === String(reference));
