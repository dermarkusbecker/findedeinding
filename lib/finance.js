...
export function financeReport(invoices,payments,{year,month=0,today}){
 ...
 const selected=invoiceBalances(invoices.filter(i=>i.status==='issued'&&i.invoice_date>=from&&i.invoice_date<=to),payments,to).sort((a,b)=>a.invoice_date.localeCompare(b.invoice_date)||String(a.invoice_number).localeCompare(String(b.invoice_number)));
 ...
 const rates=[...new Set(selected.map(i=>Number(i.vat_rate||0)))].sort((a,b)=>a-b).map(rate=>{const rows=selected.filter(i=>Number(i.vat_rate||0)===rate);return{rate,net:sum(rows,'net'),vat:sum(rows,'vat'),gross:sum(rows,'gross')};});
 return{from,to,rates,invoiceCount:selected.length,paid:sum(selected,'paid'),open:sum(selected,'open'),net:sum(selected,'net'),vat:sum(selected,'vat'),gross:sum(selected,'gross'),payments:sum(incoming,'amount'),months,invoices:selected};
}
