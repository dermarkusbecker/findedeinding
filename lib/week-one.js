...
export function createWeekOneState() {
  return {
    ...
    career_history: { cv_uploaded: false, cv_file_id: null, cv_file_name: null, cv_uploaded_at: null, stations: [], source: null, participant_confirmed: false, completed: false },
    week_summary: null,
    completed_at: null,
    updated_at: null,
  };
...
export function stepStatuses(state = createWeekOneState()) {
  ...
  return [
    ...
    { id: 'career', title: 'Lebenslauf hochgeladen', subtitle: 'Upload und Bestätigung erforderlich', status: state.career_history.cv_uploaded && state.career_history.completed ? 'completed' : state.career_history.cv_uploaded ? 'in_progress' : 'open' },
  ];
}
