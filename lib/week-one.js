...
export function validateWishClarity(value = '') {
  const normalized = normalizeWhitespace(value);
  if (!normalized) return { valid: false, reason: 'EMPTY', normalized, followup: 'Was wünschst du dir gerade konkret?' };
  const plain = normalized.toLowerCase().replace(/[.!?,;:]+$/g, '').trim();
  const vagueFollowups = [
    { pattern: /^mehr(?: davon)?$/, followup: 'Mehr wovon genau?' },
    { pattern: /^(?:mehr )?freiheit$/, followup: 'Was bedeutet Freiheit für dich gerade konkret?' },
    { pattern: /^(?:mehr )?(?:glück|glücklich(?: sein)?)$/, followup: 'Woran würdest du merken, dass du glücklicher bist?' },
    { pattern: /^(?:mehr )?(?:erfolg|geld|klarheit)$/, followup: `Was bedeutet „${normalized.replace(/[.!?,;:]+$/g, '')}“ für dich gerade konkret?` },
  ];
  const vague = vagueFollowups.find(({ pattern }) => pattern.test(plain));
  if (vague) return { valid: false, reason: 'TOO_ABSTRACT', normalized, followup: vague.followup };
  return { valid: true, reason: null, normalized, followup: null };
}
...
export function weekOnePrompt(state = createWeekOneState(), firstName = '') {
  ...
  if (state.current_step === WEEK_ONE_STEPS.WISHES) return { title: 'Deine drei Wünsche', question: 'Stell dir vor, vor dir steht eine Fee und du hast genau drei Wünsche frei. Welche drei Dinge würdest du dir für dein Leben aktuell am meisten wünschen?', help: 'Nenne einfach deine drei aktuell wichtigsten Wünsche. Kurze Antworten sind völlig in Ordnung, solange klar ist, was du dir wünschst.', type: 'wishes' };
...
export function applyWeekOneAction(inputState, action = {}) {
  ...
  if (action.type === 'correct_wish') {
    ...
    const wish = state.wishes[index];
    const validation = validateWishClarity(action.wish);
    ...
    if (!validation.valid) return fail(validation.followup, { reason: validation.reason, followup: validation.followup });
    state.wishes[index] = {
      ...blankWish(index + 1),
  ...
  if (action.type === 'save_wishes') {
    ...
    const validations = wishes.map((wish) => validateWishClarity(wish));
    if (validations.some((item) => !item.valid)) return fail(validations.find((item) => !item.valid).followup, { reason: 'INVALID_WISHES', validations });
  ...
  if (action.type === 'confirm_wishes') {
    ...
    const wishes = Array.isArray(action.wishes) ? action.wishes : [];
    const validations = wishes.map((wish) => validateWishClarity(wish));
    if (validations.length !== 3 || validations.some((item) => !item.valid)) return fail(validations.find((item) => !item.valid)?.followup || 'Bitte nenne genau drei verständliche Wünsche.', { reason: 'INVALID_WISHES', validations });
  ...
  if (action.type === 'save_target') {
    const answer = normalizeWhitespace(action.answer);
    if (!answer) return fail('Was soll sich für dich durch Finde dein Ding verändern?', { reason: 'EMPTY' });
    if (asksForCareerRecommendation(answer)) return fail('Dafür sammeln wir gerade erst die Puzzleteile. Ich möchte dir nicht zu früh eine Richtung einreden. Lass uns zuerst deine Ausgangsbasis sauber aufnehmen.', { reason: 'NO_EARLY_RECOMMENDATION' });
    state.fdd_target.raw_answer = answer;
    state.fdd_target.desired_result = answer;
    if (targetNeedsClarification(answer)) state.current_step = WEEK_ONE_STEPS.TARGET_CLARIFY;
    else { state.fdd_target.completed = true; state.current_step = state.clarity_baseline?.completed ? WEEK_ONE_STEPS.CAREER_CHOICE : WEEK_ONE_STEPS.CLARITY; }
    return finish();
  }
  if (action.type === 'clarify_target') {
    ...
    state.current_step = state.clarity_baseline?.completed ? WEEK_ONE_STEPS.CAREER_CHOICE : WEEK_ONE_STEPS.CLARITY;
    return finish();
  }
  if (action.type === 'save_clarity') {
    ...
    if (state.current_step === WEEK_ONE_STEPS.CLARITY) state.current_step = WEEK_ONE_STEPS.CAREER_CHOICE;
    return finish();
  }
