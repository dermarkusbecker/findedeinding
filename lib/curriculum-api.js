...
import {isCurriculum,curriculumQuery,curriculumRecords,respondCurriculum,evidenceStrength} from './curriculum-runtime.js';
export async function handleCurriculum(request,response,session,result){
 ...
 if(request.method==='GET')return response.status(200).json({version,definition,records});
 ...
 const lesson=w?.steps.find(s=>s.id===body.stepId);
 ...
 if(body.action==='confirm'){
  ...
  payload={...payload,status:'completed',ready:true,signals:payload.signals.map(s=>{const next={...s,participant_status:['bestätigt','abgeschwächt','widersprochen','offen'].includes(body.signalStatuses?.[s.signal])?body.signalStatuses[s.signal]:'offen'};return {...next,strength:evidenceStrength(next)};})};
  ...
  if(lesson.id==='c44_w1_l4'){
   ...
   if(cv[0])context.unshift({week:0,step_id:'optional_cv',status:'completed',messages:[{role:'user',source_id:`document_${cv[0].id}`,content:String(cv[0].extracted_text||JSON.stringify(cv[0].extracted_data||{})).slice(0,12000)}],summary:'Optionaler Lebenslauf: Hintergrund, keine Berufsdiagnose.',structured_data:{},signals:[]});
  }
 ...
 const saved=await curriculumQuery(result.service,'rpc/save_curriculum_lesson','POST',{p_user:session.participantId,p_version:version,p_week:week,p_step:lesson.id,p_revision:Number(body.revision||0),p_payload:payload});
