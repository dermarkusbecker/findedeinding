...
export function buildProgressCelebration({
  ...
  const safeClarityHistory = Array.isArray(clarityHistory) ? clarityHistory : [];
  const finishedWeeks = [...new Set(safeCompletedWeeks.map(Number).filter((week) => week >= 1 && week <= 8))];
  const measurements = safeClarityHistory.filter((item) => validScore(item?.score));
  ...
  return {
    ...
    claraTitle,
    claraCopy,
    ctaLabel: isComplete ? 'Meine Entwicklung ansehen →' : onboardingComplete ? `In Woche ${week} weitermachen →` : 'Onboarding fortsetzen →',
    isComplete,
  };
}
