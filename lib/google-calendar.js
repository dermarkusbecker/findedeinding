...
export function authorizationUrl(profileId) {
  ...
  const query = new URLSearchParams({ client_id: config.clientId, redirect_uri: config.redirectUri, response_type: 'code', scope: GOOGLE_OAUTH_SCOPES.join(' '), access_type: 'offline', prompt: 'consent', include_granted_scopes: 'true', state: createOAuthState(profileId) });
...
export async function calendarBusyIntervals(accessToken, start, end) {
...
export async function assertCalendarAvailable(accessToken, start, end) {
  if ((await calendarBusyIntervals(accessToken, start, end)).length) throw Object.assign(new Error('Der gewählte Zeitraum ist im Google-Kalender bereits belegt.'), { status: 409 });
}

function eventBody(lead, start, end, includeConference) {
  const body = {
    summary: `${lead.appointment_title || "Finde dein Ding – Erstgespräch"} mit ${lead.name}`,
    description: `CRM-Lead: ${lead.name}\nE-Mail: ${lead.email}\nMobil: ${lead.mobile_phone || lead.phone || '—'}\nTelefon: ${lead.phone || '—'}`,
...
export async function saveCalendarEvent(accessToken, lead, start, end, { notifyAttendees = true } = {}) {
  const encodedCalendar = encodeURIComponent('primary');
  const sendUpdates = notifyAttendees ? 'all' : 'none';
  if (lead.calendar_event_id) {
    return googleRequest(`calendars/${encodedCalendar}/events/${encodeURIComponent(lead.calendar_event_id)}?conferenceDataVersion=1&sendUpdates=${sendUpdates}`, accessToken, { method: 'PATCH', body: JSON.stringify(eventBody(lead, start, end, false)) });
  }
  return googleRequest(`calendars/${encodedCalendar}/events?conferenceDataVersion=1&sendUpdates=${sendUpdates}`, accessToken, { method: 'POST', body: JSON.stringify(eventBody(lead, start, end, true)) });
}
