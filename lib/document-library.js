...
export function buildDocumentLibrary(program = {}, workspace = {}) {
  ...
  const configuredMilestones = program.processVersion?.definition ? Object.fromEntries(program.processVersion.definition.weeks.map((week,index) => [index+1, {title:week.title,requirement:`Nach Bearbeitung und Auswertung deiner Aufgaben in Woche ${index+1}`}])) : milestones;
  const seen = new Set();
  ...
  return {
    ...
    weeks: weeks.map((week) => ({ ...week, milestone: configuredMilestones[week.week], daysAfterStart: (week.week - 1) * 7, documents: documents.filter((doc) => !isGeneral(doc) && Number(doc.week) === week.week) })),
...
export function renderDocumentLibrary(model, { generalOnly = false } = {}) {
  ...
  if (generalOnly) return `<section class="library-general" aria-labelledby="libraryGeneralTitle"><div class="library-section-heading"><div><p>DEIN FUNDAMENT</p><h2 id="libraryGeneralTitle">Deine persönlichen Unterlagen</h2></div><span>${model.groups.reduce((sum,group)=>sum+group.documents.length,0)} Dokumente</span></div><div class="library-general-grid">${groups}</div>${contracts}</section>`;
...
export function renderInsightDocuments(model) {
  const weeks = model.weeks.filter(week => week.documents.length);
  if (!weeks.length) return '';
  return `<section class="insight-files"><p class="eyebrow">Ergänzend zu deinen Reflexionen</p><h2>Deine Auswertungen & Wochenunterlagen</h2><div class="insight-files-grid">${weeks.map(week=>`<section class="insight-file-group"><h3>Woche ${week.week} · ${escape(week.summary.title)}</h3>${week.documents.map(documentButton).join('')}</section>`).join('')}</div></section>`;
}
