...
import { isProgramWeekFinalized } from '../program-access.js';
...
export async function handleParticipantDocument(request, response) {
  ...
  try {
    ...
    if (isProgramWeekFinalized(program.progress, normalizedWeek)) return response.status(409).json({ error: `Woche ${normalizedWeek} ist abgeschlossen. Dokumente können hier nicht mehr verändert werden.` });
