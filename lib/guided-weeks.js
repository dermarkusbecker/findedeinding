...
export const GUIDED_WEEK_DEFINITIONS = Object.freeze({
  ...
  7: {
    ...
    steps: [
      ...
      step('future_timelines', 'Zwei Wege, zwei Zukünfte', 'Schreibe beide Zukunftswege im Workbook auf und lade die Seiten anschließend hoch.', 'decision_or_escalation', { kind: 'upload', documentType: 'workbook' }),
      step('decision', 'Tendenz oder Entscheidung', 'Hat sich durch die beiden Zukunftswege eine klare Tendenz oder Entscheidung ergeben?', 'decision_or_escalation', { allowNone: true }),
      step('decision_resolution', 'Entscheidung oder Coach-Eskalation', 'Eine klare Entscheidung oder die Übergabe an Markus wird sicher bestätigt.', 'decision_or_escalation', { kind: 'external', external: 'decision_or_escalation' }),
    ],
  },
  8: {
...
export function createGuidedWeekState(week) {
  ...
  return { version: 1, week: Number(week), status: 'in_progress', current_step: definition.steps[0].id, answers: {}, completed_steps: [], documents: {}, external_results: {}, updated_at: null, completed_at: null };
}
...
export function normalizeGuidedWeekState(week, state) {
  ...
  return { ...base, ...state, answers: state.answers || {}, completed_steps: Array.isArray(state.completed_steps) ? state.completed_steps : [], documents: state.documents || {}, external_results: state.external_results || {} };
}
...
export function applyGuidedWeekAction(inputState, action = {}) {
  ...
  if (!definition || !active) return { ok: false, error: 'Der aktuelle Wochenschritt ist ungültig.', state: inputState };
...
export function guidedStepStatuses(state) {
  ...
  return guidedWeekDefinition(normalized.week).steps.map((item) => ({ id: item.id, title: item.title, status: normalized.completed_steps.includes(item.id) ? 'completed' : item.id === normalized.current_step ? 'in_progress' : 'open', kind: item.kind }));
}
...
export function guidedWeekComplete(state) {
  const normalized = normalizeGuidedWeekState(state?.week, state);
  return guidedWeekDefinition(normalized.week).steps.every((item) => normalized.completed_steps.includes(item.id));
}
