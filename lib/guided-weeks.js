...
export function applyGuidedWeekAction(inputState, action = {}) {
  ...
  if (action.type === 'correct_answer') {
    ...
    if (!target || !state.completed_steps.includes(target.id) || ['external', 'upload', 'scale', 'priority_selection'].includes(target.kind)) return { ok: false, error: 'Diese frühere Antwort kann hier nicht korrigiert werden.', state: inputState };
