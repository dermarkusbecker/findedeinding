...
export const MOTIVATOR_OPTIONS = Object.freeze([
  'Macht', 'Freiheit', 'Neugier', 'Anerkennung', 'Ordnung', 'Sparen', 'Ehre',
  'Gerechtigkeit', 'Beziehungen', 'Status', 'Familie', 'Erfolg', 'Genuss',
  'Lebenskraft', 'Schönheit', 'Spaß', 'Ruhe', 'Reichtum', 'Harmonie',
  'Herausforderung', 'Ruhm', 'Freude', 'Idealismus', 'Sicherheit', 'Abenteuer',
...
export const GUIDED_WEEK_DEFINITIONS = Object.freeze({
  ...
  3: {
    ...
    steps: [
      step('motivators', 'Top 5 Motivatoren', 'Wähle genau fünf Motivatoren aus und bringe sie anschließend in deine persönliche Reihenfolge – Platz 1 ist dir am wichtigsten.', 'motivators', { kind: 'priority_selection', options: MOTIVATOR_OPTIONS, minItems: 5, maxItems: 5 }),
...
export function guidedWeekDefinition(week, state = null) { return state?.definition || GUIDED_WEEK_DEFINITIONS[Number(week)] || null; }

export function createGuidedWeekState(week, configured = null) {
  const definition = configured || guidedWeekDefinition(week);
  if (!definition) return null;
  return { ...(configured ? { definition: configured } : {}), version: 2, week: Number(week), status: 'in_progress', current_step: definition.steps[0].id, clarity_checkin: { score: null, changed: null, note: '', completed: false, recorded_at: null }, answers: {}, completed_steps: [], documents: {}, external_results: {}, week_reflection: null, updated_at: null, completed_at: null };
}

export function normalizeGuidedWeekState(week, state) {
  const base = createGuidedWeekState(week, state?.definition);
...
export function currentGuidedStep(state) {
  return guidedWeekDefinition(state?.week, state)?.steps.find((item) => item.id === state.current_step) || null;
}
...
function validateGuidedAnswer(stepDefinition, action) {
  ...
  if (stepDefinition.kind === 'scale') {
    ...
    return Number.isInteger(score) && score >= stepDefinition.min && score <= stepDefinition.max ? { ok: true, answer: String(score), items: [] } : { ok: false, error: `Bitte wähle einen Wert zwischen ${stepDefinition.min} und ${stepDefinition.max}.` };
  }
  if (['priority_selection','selection'].includes(stepDefinition.kind)) {
    const allowed = new Set(stepDefinition.options || []);
    if ((items.length < stepDefinition.minItems || items.length > (stepDefinition.maxItems || stepDefinition.minItems)) || new Set(items).size !== items.length || items.some((item) => !allowed.has(item))) {
      return { ok: false, error: `Bitte wähle ${stepDefinition.minItems === stepDefinition.maxItems ? `genau ${stepDefinition.minItems}` : `${stepDefinition.minItems} bis ${stepDefinition.maxItems}`} unterschiedliche Einträge aus der vorgegebenen Auswahl.` };
    }
    return { ok: true, answer: items.map((item, index) => `${index + 1}. ${item}`).join('\n'), items };
  }
  if (!answer && !(stepDefinition.allowNone && action.confirmedNone)) return { ok: false, error: 'Bitte beantworte zuerst die aktuelle Frage.' };
  if (stepDefinition.expected && answer.toLocaleUpperCase() !== stepDefinition.expected.toLocaleUpperCase()) return { ok: false, error: `Schreibe ${stepDefinition.expected}, sobald du bereit bist.` };
...
export function applyGuidedWeekAction(inputState, action = {}) {
  ...
  if (action.type === 'correct_answer') {
    ...
    if (state.status === 'completed' || state.completed_at || !target || !state.completed_steps.includes(target.id) || ['external', 'upload', 'scale'].includes(target.kind)) return { ok: false, error: 'Diese frühere Antwort kann hier nicht korrigiert werden.', state: inputState };
...
export function guidedStepStatuses(state) {
  ...
  return [
    ...
    ...guidedWeekDefinition(normalized.week, normalized).steps.map((item) => ({ id: item.id, title: item.title, status: normalized.completed_steps.includes(item.id) ? 'completed' : !normalized.clarity_checkin?.completed ? 'open' : item.id === normalized.current_step ? 'in_progress' : 'open', kind: item.kind })),
  ];
}

export function guidedGateStatus(state) {
  const normalized = normalizeGuidedWeekState(state?.week, state);
  const definition = guidedWeekDefinition(normalized.week, normalized);
...
export function guidedWeekComplete(state) {
  const normalized = normalizeGuidedWeekState(state?.week, state);
  return Boolean(normalized.clarity_checkin?.completed) && guidedWeekDefinition(normalized.week, normalized).steps.every((item) => normalized.completed_steps.includes(item.id));
}
