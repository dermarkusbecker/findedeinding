...
export function createGuidedWeekState(week) {
  ...
  return { version: 2, week: Number(week), status: 'in_progress', current_step: definition.steps[0].id, clarity_checkin: { score: null, changed: null, note: '', completed: false, recorded_at: null }, answers: {}, completed_steps: [], documents: {}, external_results: {}, week_reflection: null, updated_at: null, completed_at: null };
}
...
function validateGuidedAnswer(stepDefinition, action) {
  ...
  if (stepDefinition.kind === 'priority_selection') {
    const allowed = new Set(stepDefinition.options || []);
    if (items.length !== stepDefinition.minItems || new Set(items).size !== items.length || items.some((item) => !allowed.has(item))) {
      return { ok: false, error: `Bitte wähle genau ${stepDefinition.minItems} unterschiedliche Motivatoren aus der vorgegebenen Auswahl und ordne sie nach Priorität.` };
    }
