...
export async function provisionProgramUser(config, { name, email: emailInput, phone = '', startDate, permissions = ['customer_portal', 'clara_program', 'documents'] }) {
  ...
  const profilePayload = { auth_user_id: authUserId, name, email, phone: String(phone || '').trim().slice(0, 40) || null, role: 'user', status: 'active', permissions: USER_PERMISSIONS.filter((permission) => permissions.includes(permission)), must_change_password: true, one_time_password_issued_at: new Date().toISOString() };
