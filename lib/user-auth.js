...
async function createProfile(config, authUserId, email, metadata) {
  ...
  if (metadata.createProgram) {
    ...
    if (!progress[0]) await Promise.all([
      json(await fetch(`${config.url}/rest/v1/participant_progress`, { method: 'POST', headers: authHeaders(config.serviceKey), body: JSON.stringify({ user_profile_id: profile.id, process_status: 'ONBOARDING', current_week: 1, program_start_date: start, access_mode: 'time_based', program_status: 'active' }) })),
