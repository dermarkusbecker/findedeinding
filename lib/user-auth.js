...
export async function provisionProgramUser(config, { name, email: emailInput, startDate, permissions = ['customer_portal', 'clara_program', 'documents'] }) {
  ...
  if (!authUserId) {
    const authUser = await createManagedAuthUser(config, email, name);
    authUserId = authUser.id;
  } else await sendPasswordReset(config, email);
  const profilePayload = { auth_user_id: authUserId, name, email, role: 'user', status: 'active', permissions: USER_PERMISSIONS.filter((permission) => permissions.includes(permission)) };
  ...
  if (!progress[0] && profile.permissions.includes('clara_program')) {
    ...
    await Promise.all([
      json(await fetch(`${config.url}/rest/v1/participant_progress`, { method: 'POST', headers: authHeaders(config.serviceKey), body: JSON.stringify({ user_profile_id: profile.id, process_status: 'ONBOARDING', current_week: 1, program_start_date: programStartDate, access_mode: 'completion_based', program_status: 'active' }) })),
      json(await fetch(`${config.url}/rest/v1/week_gates`, { method: 'POST', headers: authHeaders(config.serviceKey), body: JSON.stringify(participantGateRows(profile.id)) })),
    ]);
  }
  return profile;
