import {sendLoggedRecovery} from './system-mail-log.js';
...
export async function profileByAuthId(config, authUserId) {
  const response = await fetch(`${config.url}/rest/v1/user_profiles?auth_user_id=eq.${encodeURIComponent(authUserId)}&select=id,auth_user_id,name,email,role,status,permissions,staff_role,staff_permissions,portal_username,customer_number,must_change_password,one_time_password_issued_at,access_invite_sent_at,password_changed_at&limit=1`, { headers: authHeaders(config.serviceKey) });
  const rows = await json(response);
  return rows[0] || null;
}

export async function profileById(config, profileId) {
  const response = await fetch(`${config.url}/rest/v1/user_profiles?id=eq.${encodeURIComponent(profileId)}&select=id,auth_user_id,name,email,role,status,permissions,staff_role,staff_permissions,portal_username,customer_number,must_change_password,one_time_password_issued_at,access_invite_sent_at,password_changed_at&limit=1`, { headers: authHeaders(config.serviceKey) });
  const rows = await json(response);
  return rows[0] || null;
}
...
async function currentProfile(request, response) {
  ...
  try {
    if(session.mobileSessionId){const {activeMobileSession}=await import('./mobile-api.js');if(!await activeMobileSession(config,session)){response.status(401).json({error:'App-Sitzung abgelaufen.'});return null;}}
    ...
    return { ...session, role: profile.role, permissions: profile.role === 'admin' ? USER_PERMISSIONS : (profile.permissions || []), staffRole: profile.staff_role, staffPermissions: profile.staff_permissions?.length ? profile.staff_permissions : staffPermissionsFor(profile.staff_role), profile };
  } catch (error) { response.status(error.status || 500).json({ error: error.message }); return null; }
}

export async function requireCurrentAdmin(request, response, permission = '') {
  const session = sessionFromRequest(request);
  ...
  if (required.length && !required.some((item) => current.staffPermissions.includes(item))) { response.status(403).json({ error: 'Für diese CRM-Funktion fehlt deiner Rolle die Berechtigung.' }); return null; }
  return current;
}
...
function knownBootstrap(email, password) {
  ...
  if (email === adminIdentity && password === process.env.ADMIN_PASSWORD) return { name: 'Markus Becker', role: 'admin', permissions: USER_PERMISSIONS, staffRole: 'owner', staffPermissions: staffPermissionsFor('owner') };
  return null;
}
...
async function createProfile(config, authUserId, email, metadata) {
  ...
  const response = await fetch(`${config.url}/rest/v1/${path}`, {
    ...
    body: JSON.stringify({ auth_user_id: authUserId, name: metadata.name, email, role: metadata.role, status: 'active', permissions: metadata.permissions, staff_role: metadata.staffRole || null, staff_permissions: metadata.staffPermissions || [] }),
  });
  const rows = await json(response);
  return rows[0];
}
...
export async function provisionProgramUser(config, { name, email: emailInput, phone = '', startDate, sourceLeadId = null, permissions = ['customer_portal', 'clara_program', 'documents'], sendInvitation = true }) {
  ...
  const lookup = await json(await fetch(`${config.url}/rest/v1/user_profiles?email=eq.${encodeURIComponent(email)}&select=id,auth_user_id,name,email,role,status,permissions,source_lead_id&limit=1`, { headers: authHeaders(config.serviceKey) }));
  ...
  const normalizedPhone = String(phone || '').trim().slice(0, 40) || null;
  const profilePayload = { auth_user_id: authUserId, name, email, phone: normalizedPhone, mobile_phone: normalizedPhone, whatsapp_phone: normalizedPhone, whatsapp_same_as_mobile: true, source_lead_id: sourceLeadId || profile?.source_lead_id || null, role: 'user', status: 'active', permissions: USER_PERMISSIONS.filter((permission) => permissions.includes(permission)), must_change_password: true, one_time_password_issued_at: new Date().toISOString() };
  ...
  if (!progress[0] && profile.permissions.includes('clara_program')) {
    ...
    await Promise.all([
      json(await fetch(`${config.url}/rest/v1/participant_progress`, { method: 'POST', headers: authHeaders(config.serviceKey), body: JSON.stringify({ user_profile_id: profile.id, process_status: 'ONBOARDING', current_week: 0, program_start_date: programStartDate, access_mode: 'time_based', program_status: 'active' }) })),
  ...
  if (!sendInvitation) return { ...profile, oneTimePassword };
  await sendPasswordReset(config, email, {purpose:'invitation'});
...
export async function sendPasswordReset(config, emailInput, options = {}) {
  const email = cleanEmail(emailInput);
  if (!validEmail(email)) throw Object.assign(new Error('Bitte eine gültige E-Mail-Adresse eingeben.'), { status: 400 });
  await sendLoggedRecovery(config,email,async()=>json(await fetchRecovery()),options);
  async function fetchRecovery(){return fetch(`${config.url}/auth/v1/recover`, { method: 'POST', headers: authHeaders(config.anonKey), body: JSON.stringify({ email }) });}
}
