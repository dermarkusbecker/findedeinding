...
export function validateProgramDefinition(input) {
  ...
  if(input.engine!==undefined&&input.engine!=='curriculum_4plus4')errors.push('Unbekannte Prozesslogik.');
  if(input.modules!==undefined&&(!Array.isArray(input.modules)||input.modules.length>2000))errors.push('Modulbibliothek ist ungültig oder zu groß.');
  const ids = new Set(),
    weekIds = new Set();
  input.weeks.forEach((week, index) => {
    ...
    if(input.engine==='curriculum_4plus4'&&week.steps.every(s=>s?.optional))errors.push(`${prefix}: Mindestens eine Pflichtlektion ist erforderlich.`);
    week.steps.forEach((step) => {
      ...
      if (
        ...
        ids.has(step.id) ||
        (step.id === "weekly_clarity" || String(step.id).startsWith("week_complete_"))
      )
      ...
      if(input.engine==='curriculum_4plus4'){
        if(step.method!==undefined&&!['evidence_dialog','matrix','timeline','research','hypothesis','proof','action_plan','synthesis'].includes(step.method))errors.push(`${label}: Unbekannte Lektionsmethode.`);
        if(step.optional!==undefined&&typeof step.optional!=='boolean')errors.push(`${label}: Optional muss ein Wahrheitswert sein.`);
      }
      for (const field of ["guidance", "completionCriteria", "learningGoal", "opening", "adaptiveLogic", "storageLogic", "evaluation", "note"])
        if (
  ...
  if(input.engine === "curriculum_4plus4") return [];
...
export function moveProgramTask(
  ...
  if (
    fromWeek < (definition.engine === "curriculum_4plus4" ? 0 : 1) ||
    toWeek < (definition.engine === "curriculum_4plus4" ? 0 : 1) ||
