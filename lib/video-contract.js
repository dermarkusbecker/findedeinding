...
export async function buildVideoContractPdf(contract, { videoConfirmed = false, customerSigned = false, providerConfirmed = false, draft = false } = {}) {
  ...
  for (const [name, checked] of Object.entries(checks)) setCheck(form, name, Boolean(checked && (videoConfirmed || draft || !name.startsWith('v'))));
  ...
  if (draft) for (const page of pdf.getPages()) page.drawText('ENTWURF - zwischengespeichert, nicht abgeschlossen', {x:30,y:12,size:7,font,color:rgb(.55,.3,.15)});
