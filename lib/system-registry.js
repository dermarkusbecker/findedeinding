...
export function buildSystemRegistry({ googleConfigured = false, googleConnection = null, openaiConfigured = false, openaiModel = 'gpt-5.6-terra', whatsappConfigured = false, checkedAt = new Date().toISOString() } = {}) {
  ...
  const agents = [
    ...
    },
    {
      ...
      situation: 'Verdichtet ausschließlich die final gespeicherten Aussagen der beendeten Woche zu einem persönlichen Rückblick.',
      tool: `OpenAI Responses · ${openaiModel}`, detail: openaiConfigured ? 'Strukturierte Reflexion mit beleggebundener Ersatzlogik ist aktiv.' : aiDetail, status: aiState,
    },
    {
