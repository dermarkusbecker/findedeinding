...
export function buildSystemRegistry({ mailConfigured = false, mailUser = '', googleConfigured = false, googleConnection = null, googleMeetRecordingReady = false, openaiConfigured = false, openaiModel = 'gpt-5.6-terra', whatsappConfigured = false, checkedAt = new Date().toISOString() } = {}) {
  ...
  const integrations = [
    ...
    },
    {
      ...
      purpose: 'Versorgt Clara, Situationsanalyse, Wochenreflexionen und Dokumentstrukturierung mit KI-Funktionen.',
    ...
    },
    {
      ...
      purpose: 'Prüft den sicheren Zugang zum Postfach für Versand (SMTP) und Empfang (IMAP).',
      detail: mailConfigured ? `${mailUser} · Zugangsdaten hinterlegt. Die Anmeldung muss noch geprüft werden. CRM-Versand und Synchronisierung sind noch nicht aktiviert.` : 'Postfachadresse und Passwort müssen als STRATO_MAILBOX_USER und STRATO_MAILBOX_PASSWORD in Vercel hinterlegt werden.',
      status: mailConfigured ? status('ready', 'Zugang prüfen', 'warning') : PLANNED,
