...
export function configuredWeekState(week, state, processVersion) {
  ...
  if (!definition || processVersion?.definition?.engine === 'curriculum_4plus4' || Number(week) === 1) return state;
  return {
    ...state,
...
export async function handleProgramBuilder(
  ...
  if (request.method === "GET") {
    const requested=request.query?.draftId;
    if(requested&&!/^[0-9a-f-]{36}$/i.test(requested))return response.status(400).json({error:'Ungültiger Entwurf.'});
    const [draft, versions, snapshots, selected] = await Promise.all([
      ...
      query(service,"program_builder_snapshots?select=id,name,revision,created_at&order=created_at.desc"),
      requested?query(service,`program_builder_snapshots?id=eq.${requested}&select=definition,name`):Promise.resolve([]),
    ]);
    if(requested&&!selected.length)return response.status(404).json({error:'Entwurf nicht gefunden.'});
    return response
      .status(200)
      .json({
        definition: selected[0]?.definition || draft[0]?.definition || defaultProgramDefinition(overrides),
        snapshots,
        selectedName:selected[0]?.name||null,
  ...
  const result = await query(service, "rpc/save_named_program_draft", {
    method: "POST",
    body: JSON.stringify({
      p_name:typeof request.body?.name==='string'?request.body.name.slice(0,160):'',
