import {normalizeAppointmentCategories} from './appointment-categories.js';
...
export function normalizeBookingSettings(value = {}) {
  ...
  const offeredSource = value.offeredDurations ?? value.offered_durations;
  const offeredDurations = [...new Set((Array.isArray(offeredSource) ? offeredSource : [defaultDurationMinutes]).map(Number).filter((duration) => ALLOWED_DURATIONS.includes(duration)))].sort((a, b) => a - b);
  if (!offeredDurations.length) throw Object.assign(new Error('Mindestens eine angebotene Termindauer ist erforderlich.'), { status: 400 });
  const normalizedDefaultDuration = offeredDurations.includes(defaultDurationMinutes) ? defaultDurationMinutes : offeredDurations[0];
  return {
    ...(value.categories?{categories:normalizeAppointmentCategories(value.categories)}:{}),
    timezone: 'Europe/Berlin',
    weeklyAvailability,
    slotIntervalMinutes,
    defaultDurationMinutes: normalizedDefaultDuration,
    offeredDurations,
...
export function generateAvailableSlots({ settings: rawSettings, from, to, duration, busyIntervals = [], now = new Date() }) {
  ...
  if (!settings.offeredDurations.includes(durationMinutes)) throw Object.assign(new Error('Diese Termindauer wird aktuell nicht angeboten.'), { status: 400 });
...
export function isWithinBookingAvailability(startValue, duration, rawSettings, now = new Date()) {
  ...
  if (Number.isNaN(start.getTime()) || !ALLOWED_DURATIONS.includes(durationMinutes) || !settings.offeredDurations.includes(durationMinutes)) return false;
  const earliest = new Date(now.getTime() + settings.minNoticeHours * 3600000);
  const latest = new Date(now.getTime() + settings.bookingHorizonDays * 86400000);
