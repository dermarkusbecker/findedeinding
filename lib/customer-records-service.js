...
export async function handleCustomerRecords(request, response) {
  ...
  const required = (action === 'whatsapp-send' || action.startsWith('communication-')) ? ['communications', 'customers'] : action.startsWith('document') || action === 'avatar-upload' ? ['customers', 'program'] : ['customers', 'finance', 'communications', 'sales_calls'];
