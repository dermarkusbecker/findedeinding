...
export async function customerData(service, participantId) {
  ...
  const [documents,appointments,contracts,payments,invoices,bankAccounts]=await Promise.all([
  ...
  const balances=invoiceBalances(invoices,payments,today),sum=(items,key)=>items.reduce((total,item)=>total+cents(item[key]),0)/100;
  ...
  const openBalance=sum(balances.filter(item=>item.status==='issued'),'open');
