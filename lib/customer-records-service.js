...
export async function customerData(service, participantId) {
  ...
  const planAll=async path=>{const result=[];for(let offset=0;;offset+=500){const page=await rows(await fetch(`${service.url}/rest/v1/${path}&limit=500&offset=${offset}`,{headers:headers(service.key)}),'Ratenpläne konnten nicht geladen werden.');result.push(...page);if(page.length<500)return result;}};
  const plans=await loadPaymentPlans({all:planAll,customerId:participantId});
  const account=customerAccount({invoices,payments,events:accountEvents,today,plans});
