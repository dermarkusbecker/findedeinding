...
export async function customerData(service, participantId) {
  ...
  const lifecycleFilter = profile.source_lead_id ? `id=eq.${encodeURIComponent(profile.source_lead_id)}` : `converted_user_profile_id=eq.${id}`;
  const leadRows = await rows(await fetch(`${service.url}/rest/v1/leads?${lifecycleFilter}&select=*&limit=1`, { headers: headers(service.key) }), 'Verknüpfte Kundenakte konnte nicht geladen werden.');
  ...
  const communicationFilter=lead?`or=(lead_id.eq.${encodeURIComponent(lead.id)},user_profile_id.eq.${id})`:`user_profile_id=eq.${id}`;
  const communications=await rows(await fetch(`${service.url}/rest/v1/lead_communications?${communicationFilter}&select=*&order=occurred_at.desc`,{headers:headers(service.key)}),'Kommunikation konnte nicht geladen werden.');
  const bankAccounts = lead ? await rows(result[4], 'Kontodaten konnten nicht geladen werden.') : [];
...
export async function handleCustomerRecords(request, response) {
  ...
  const required = action === 'customer-notes' ? ['customers','program'] : (action === 'whatsapp-send' || action.startsWith('communication-')) ? ['communications', 'customers'] : action.startsWith('document') || action === 'avatar-upload' ? ['customers', 'program'] : ['customers', 'finance', 'communications', 'sales_calls'];
