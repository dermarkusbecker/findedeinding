...
export async function getParticipantProgramAccess(participantId, now = new Date()) {
  ...
  const access = reconcileAccessFromEntries({ access: scheduledAccess, progress: progressRows[0], entries: stateEntries });
  return { service, processVersion, profile: profiles[0], progress: progressRows[0], gates, stateEntries, access, serializedAccess: serializeProgramAccess(access) };
}
