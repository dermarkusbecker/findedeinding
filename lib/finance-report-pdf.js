...
export async function financeReportPdf(report,{details=false}={}){
 ...
 section('03  Einordnung der Zahlen');note('Rechnungsumsatz: ausgestellte Rechnungen abzüglich Rechnungskorrekturen (Gutschriften) nach Belegdatum im gewählten Zeitraum. Zahlungseingänge: gebuchte Zahlungen nach Buchungsdatum im selben Zeitraum, auch zu Rechnungen anderer Monate. Beide Werte sind getrennt zu betrachten.');note(`Zahlungsstand der hier ausgewiesenen Rechnungen zum ${date(report.to)}: zugeordnet ${currency(report.paid)} · offen ${currency(report.open)}. Nicht zugeordnete Zahlungen reduzieren den offenen Rechnungsbetrag erst nach Zuordnung.`);note('Diese Übersicht enthält keine Betriebsausgaben und keine Umsatzsteuer-Voranmeldung. Sie dient der Abstimmung des CRM-Rechnungsregisters mit der Steuerberatung.');
 if(report.writeoffs?.length){section('04  Ausbuchungen');note(`Ausgebuchte Forderungen: ${currency(report.writeoffs.reduce((sum,e)=>sum+Number(e.amount),0))}. Separat ausgewiesen; keine Zahlung und keine automatische Umsatzsteuerkorrektur.`);if(details)table([{label:'Beleg / Datum',width:170},{label:'Grund',width:C-300},{label:'Betrag',width:130,right:true}],report.writeoffs.map(e=>[`${e.document_number}\n${date(e.booked_at)}`,e.reason,currency(e.amount)]));}
