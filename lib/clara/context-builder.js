...
import { resolveClarityPrompt } from '../clarity-questions.js';
...
export function buildClaraContext({ participantId, participantName, week = 1, state, messages = [], memories = [], rawEntries = [], commitment = null, questionOverrides = [] }) {
  ...
  const prompt = { ...basePrompt, question: resolveClarityPrompt(questionOverrides, week, state.current_step, basePrompt.question || '') };
