...
export function buildClaraContext({ participantId, participantName, week = 1, state, messages = [], memories = [], rawEntries = [], commitment = null }) {
  const guided = Number(week) >= 2 ? guidedWeekDefinition(week) : null;
  const activeGuidedStep = guided ? currentGuidedStep(state) : null;
  const prompt = guided ? { title: activeGuidedStep?.title || 'Wochenabschluss', question: activeGuidedStep?.question || '', type: activeGuidedStep?.kind || 'review', rules: activeGuidedStep || null, weekIntro: guided.intro } : weekOnePrompt(state, participantName?.split(' ')[0] || '');
  ...
  return {
    ...
    week,
    state,
    openTask: { prompt, statuses: guided ? guidedStepStatuses(state) : stepStatuses(state) },
