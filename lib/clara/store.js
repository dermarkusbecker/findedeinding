import { participantProcessVersion, configuredWeekState } from '../program-builder-service.js';
import { serviceHeaders } from '../program-access-service.js';
import { createWeekOneState } from '../week-one.js';
import { createGuidedWeekState, normalizeGuidedWeekState } from '../guided-weeks.js';
...
export async function readClaraData(service, participantId, week = 1) {
  ...
  const [storedMessages, storedMemories, rawEntries, states] = await Promise.all([
    ...
    fetch(`${service.url}/rest/v1/process_entries?user_profile_id=eq.${id}&week=eq.${week}&data_block=eq.week_${week}_state&select=structured_data&order=created_at.desc&limit=1`, { headers }).then((r) => read(r, `Woche ${week} konnte nicht geladen werden.`)),
  ]);
  const fallbackMessages = rawEntries.filter((entry) => [`week_${week}_clara_dialog`, `week_${week}_clara_assistant`].includes(entry.data_block)).map((entry) => ({ id: entry.id, role: entry.structured_data?.clara?.role || (entry.data_block.endsWith('assistant') ? 'assistant' : 'participant'), content: entry.raw_answer, model_response_id: entry.structured_data?.clara?.model_response_id || null, uiAction: entry.structured_data?.clara?.ui_action || null, created_at: entry.created_at }));
  ...
  const processVersion = await participantProcessVersion(service, participantId);
  const storedState = configuredWeekState(week, states[0]?.structured_data?.[`week_${week}`] || (Number(week) === 1 ? createWeekOneState() : createGuidedWeekState(week)), processVersion);
  return { processVersion, messages, previousResponseId, memories: (storedMemories || []).reverse(), rawEntries: rawEntries.reverse(), state: Number(week) === 1 ? (storedState || createWeekOneState()) : normalizeGuidedWeekState(week, storedState || createGuidedWeekState(week)) };
}
...
export async function findParticipantMessage(service, participantId, clientMessageId) {
  ...
  const fallback = await fetch(`${service.url}/rest/v1/process_entries?user_profile_id=eq.${encodeURIComponent(participantId)}&data_block=like.week_*_clara_dialog&select=id,structured_data&order=created_at.desc&limit=80`, { headers: serviceHeaders(service.key) });
...
export async function saveGuidedWeekSnapshot(service, participantId, week, state, rawAnswer) {
  const response = await fetch(`${service.url}/rest/v1/process_entries`, { method: 'POST', headers: serviceHeaders(service.key), body: JSON.stringify({ user_profile_id: participantId, week, data_block: `week_${week}_state`, raw_answer: String(rawAnswer || '').slice(0, 10000) || null, structured_data: { [`week_${week}`]: state }, evidence_level: 'participant_statement' }) });
  if (!response.ok) throw new Error(`Claras validierter Fortschritt in Woche ${week} konnte nicht gespeichert werden.`);
}
