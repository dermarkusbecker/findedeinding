...
export async function readClaraData(service, participantId, week = 1) {
  ...
  const [storedMessages, storedMemories, rawEntries, states] = await Promise.all([
    fetch(`${service.url}/rest/v1/clara_messages?user_profile_id=eq.${id}&week=eq.${week}&select=id,role,content,model_response_id,created_at&order=created_at.desc&limit=20`, { headers }).then((r) => optionalRead(r, 'Clara-Verlauf konnte nicht geladen werden.')),
