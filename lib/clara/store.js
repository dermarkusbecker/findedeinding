...
export async function readClaraData(service, participantId, week = 1) {
  ...
  const storedState = states[0]?.structured_data?.[`week_${week}`];
  return { messages, previousResponseId, memories: (storedMemories || []).reverse(), rawEntries: rawEntries.reverse(), state: Number(week) === 1 ? (storedState || createWeekOneState()) : normalizeGuidedWeekState(week, storedState || createGuidedWeekState(week)) };
}
