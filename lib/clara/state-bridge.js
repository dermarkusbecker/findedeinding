...
import { applyGuidedWeekAction } from '../guided-weeks.js';
...
export function applyClaraStateSuggestions(state, suggestions = []) {
  if (Number(state?.week) >= 2) return applyGuidedStateSuggestions(state, suggestions);
  let current = state;
  const accepted = [];
  const rejected = [];
...
function applyGuidedStateSuggestions(state, suggestions = []) {
  let current = state;
  const accepted = [];
  const rejected = [];
  let stateChangeApplied = false;
  for (const suggestion of suggestions.slice(0, 3)) {
    if (suggestion.action === 'none') { accepted.push({ action: 'none' }); continue; }
    const isCorrection = suggestion.action === 'correct_guided_answer';
    const isCurrentAnswer = suggestion.action === 'save_guided_answer' && suggestion.payload?.step_id === current.current_step;
    if (stateChangeApplied || (!isCorrection && !isCurrentAnswer)) {
      rejected.push({ suggestion, reason: 'ACTION_NOT_ALLOWED_FOR_CURRENT_STEP' });
      continue;
    }
    const result = applyGuidedWeekAction(current, { type: isCorrection ? 'correct_answer' : 'save_answer', stepId: suggestion.payload.step_id, answer: suggestion.payload.answer, items: suggestion.payload.items, score: suggestion.payload.score, confirmedNone: suggestion.payload.confirmed_none === true });
    if (!result.ok) { rejected.push({ suggestion, reason: 'REDUCER_REJECTED', error: result.error }); continue; }
    current = result.state;
    accepted.push({ action: suggestion.action, reducerAction: { type: isCorrection ? 'correct_answer' : 'save_answer', stepId: suggestion.payload.step_id } });
    stateChangeApplied = true;
  }
  return { state: current, changed: current !== state, accepted, rejected };
}
