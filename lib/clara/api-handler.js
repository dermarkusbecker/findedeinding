...
import { applyWeekOneAction, stepStatuses, weekOneComplete, WEEK_ONE_STEPS } from '../week-one.js';
...
export async function handleClaraMessage(request, response) {
  ...
  try {
    ...
    if (request.body?.action === 'confirm_result') {
      ...
      const llm = { response: { message: 'Perfekt. Deine drei Wünsche sind festgehalten. Lass uns jetzt deinen ersten Wunsch genauer anschauen.', action: 'respond', step_status: 'completed' }, model: 'server', responseId: null, usage: null, uiAction: { type: 'complete_step', stepStatus: 'completed', confirmation: null } };
