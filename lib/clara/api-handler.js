...
export async function handleClaraMessage(request, response) {
  ...
  try {
    ...
    if (request.method === 'GET') return response.status(200).json({ messages: data.messages, week, currentStep: data.state.current_step, currentPrompt: configuredPromptForState(questionOverrides, week, data.state) });
