import { processQuestionOverrides } from '../program-builder-service.js';
...
export async function handleClaraMessage(request, response) {
  ...
  try {
    ...
    if(program.curriculum && request.method!=='GET')return response.status(409).json({error:'Bitte Clara in der aktuellen 4+4-Lektion öffnen. Frühere Gespräche bleiben gespeichert.'});
    ...
    const questionOverrides = processQuestionOverrides(data.processVersion, await readClarityQuestionOverrides(program.service, week));
