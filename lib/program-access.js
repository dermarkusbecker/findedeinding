...
const validMode = (mode) => Object.values(ACCESS_MODES).includes(mode) ? mode : ACCESS_MODES.COMPLETION;
...
export function isOnboardingComplete(progress = {}) {
  const currentWeek = Number(progress.current_week || 0);
  const status = String(progress.process_status || '').toUpperCase();
  const privacyConsent = Boolean(progress.privacy_consent_at);
  const startCommitment = Boolean(progress.start_commitment_at);
  const onboardingFinished = privacyConsent && startCommitment;
...
export function resetParticipantProgressState() {
  return {
    ...
    access_mode: ACCESS_MODES.COMPLETION,
...
export function calculateProgramAccess({ profileStatus = 'active', progress = {}, gates = [], now = new Date() } = {}) {
  ...
  const weekStates = PROGRAM_WEEKS.map((week) => {
    ...
    return { week, accessible, completed: completedWeeks.includes(week), reason, adminOverride: lockedOverrides.has(week) ? 'locked' : unlockedOverrides.has(week) ? 'unlocked' : null };
  });
