...
export function calculateProgramAccess({ profileStatus = 'active', progress = {}, gates = [], now = new Date() } = {}) {
  ...
  const processWeek = progress.process_status === 'FINAL_REPORT'
    ? 8
    : isOnboardingComplete(progress) ? Math.min(recordedProcessWeek, scheduledProcessWeek) : 0;
  const completedWeeks = finalizedWeeksFromProgress(progress).filter((week) => week < processWeek || progress.process_status === 'FINAL_REPORT');
...
export function reconcileProgramPosition(access = {}, verifiedCompletedWeeks = []) {
  ...
  const claimedWeek = Math.max(1, Math.min(8, Number(access.processWeek) || 1));
  const fullyCompleted = contiguousCompleted.length === 8 && access.recordedProcessStatus === 'FINAL_REPORT';
  const firstIncompleteWeek = fullyCompleted ? 8 : Math.min(8, contiguousCompleted.length + 1);
  const processWeek = Math.min(claimedWeek, firstIncompleteWeek);
  const completedWeeks = fullyCompleted ? contiguousCompleted : contiguousCompleted.filter((week) => week < processWeek);
  return {
    ...access,
