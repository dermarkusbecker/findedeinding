...
export function calculateProgramAccess({ profileStatus = 'active', progress = {}, gates = [], now = new Date(), fullProgramAccess = false } = {}) {
  ...
  const demoAccessActive = fullProgramAccess === true && programStatus === PROGRAM_STATUSES.ACTIVE && onboardingComplete;
  const automaticUnlockedWeeks = demoAccessActive ? [...PROGRAM_WEEKS] : timeUnlockedWeeks(progress.program_start_date, now);
  const automatic = new Set(automaticUnlockedWeeks);

  const weekStates = PROGRAM_WEEKS.map((week) => {
    let accessible = false;
    let reason = 'denied';
    if (!onboardingComplete) reason = 'onboarding_required';
    else if (programStatus !== PROGRAM_STATUSES.ACTIVE) reason = 'participant_paused';
    else if (demoAccessActive) { accessible = true; reason = 'demo_full_access'; }
    else if (automatic.has(week)) { accessible = true; reason = 'scheduled_release'; }
    else if (scheduleByWeek.has(week)) reason = 'scheduled_wait';
    const scheduledWindow = scheduleByWeek.get(week) || {};
    return { week, accessible, completed: false, readyToComplete: gateCompletedWeeks.includes(week), reason, adminOverride: null, ...scheduledWindow, unlocksAt: demoAccessActive ? progress.program_start_date : scheduledWindow.unlocksAt };
  });
  ...
  const recordedProcessWeek = Math.max(1, Math.min(8, recordedCurrentWeek || 1));
  // Ein gespeicherter Fortschrittsstand darf nie vor einer zeitlich noch
  // gesperrten Woche stehen. Das verhindert z. B. "Woche 3", solange laut
  // Projektstart ausschließlich Woche 1 freigeschaltet ist.
  const scheduledProcessWeek = currentWeek || 1;
  const processWeek = !onboardingComplete
    ? 0
    : progress.process_status === 'FINAL_REPORT' ? 8 : Math.min(recordedProcessWeek, scheduledProcessWeek);
  const completedWeeks = onboardingComplete
    ? finalizedWeeksFromProgress(progress).filter((week) => week < processWeek || progress.process_status === 'FINAL_REPORT')
    : [];
  const resolvedWeekStates = weekStates.map((state) => ({ ...state, completed: completedWeeks.includes(state.week) }));
  ...
  return {
    ...
    fullProgramAccess: demoAccessActive,
    currentWeek,
    processWeek,
    recordedCurrentWeek,
    ...
    gateCompletedWeeks,
    completedWeeks,
    weekStates: resolvedWeekStates,
    canAccessWeek: (week) => resolvedWeekStates.some((state) => state.week === Number(week) && state.accessible),
  };
}

export function reconcileProgramPosition(access = {}, verifiedCompletedWeeks = []) {
  const verified = new Set(verifiedCompletedWeeks.map(Number).filter((week) => PROGRAM_WEEKS.includes(week)));
  const contiguousCompleted = [];
  for (const week of PROGRAM_WEEKS) {
    if (!verified.has(week)) break;
    contiguousCompleted.push(week);
  }
  const fullyCompleted = contiguousCompleted.length === 8;
  const firstIncompleteWeek = fullyCompleted ? 8 : Math.min(8, contiguousCompleted.length + 1);
  const latestReleasedWeek = Math.max(1, Math.min(8, Number(access.automaticUnlockedWeeks?.at(-1)) || 1));
  const processWeek = Math.min(firstIncompleteWeek, latestReleasedWeek);
  const completedWeeks = contiguousCompleted;
  return {
    ...access,
    sourceRecordedCurrentWeek: access.recordedCurrentWeek,
    sourceRecordedProcessStatus: access.recordedProcessStatus,
    processWeek,
    recordedCurrentWeek: processWeek,
    recordedProcessStatus: fullyCompleted ? 'FINAL_REPORT' : `WEEK_${processWeek}`,
    completedWeeks,
    weekStates: (access.weekStates || []).map((state) => ({ ...state, completed: completedWeeks.includes(Number(state.week)) })),
  };
}
