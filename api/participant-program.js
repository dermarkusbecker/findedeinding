...
export default async function handler(request, response) {
  ...
  try {
    if (request.method === 'GET' && request.query?.feature === 'privacy-template') {
      ...
      response.setHeader('Content-Disposition', 'inline; filename="FDD-Datenschutzinformation-Vorschau.pdf"');
    ...
    if (request.method === 'GET') {
      ...
      const loginClarityHistory = await readLoginClarity(result.service, session.participantId);
      ...
      const profile = {
        ...
        mobilePhone: result.profile.mobile_phone,
        whatsappPhone: result.profile.whatsapp_phone,
        whatsappSameAsMobile: result.profile.whatsapp_same_as_mobile !== false,
