...
export async function openLoginClarity({manual=false}={}) {
 if(document.querySelector('.holo-checkin'))return;
 const requestId=manual?crypto.randomUUID():undefined;
 const data=await api();renderHistory(data.history);if(!data.required&&!manual)return;
 ...
 const base=dialog.querySelector('.holo-network path');for(let i=0;i<3;i++){const pulse=base.cloneNode();pulse.classList.add('holo-current');pulse.style.animationDelay=`${i*-.8}s`;dialog.querySelector('svg').append(pulse);}
